...
BOT_TOKEN = "8991433516:AAHcesPuD5RpIyyDh8TXT6IAtzB4_mDGM1A"
OPENROUTER_KEY = "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"
AGNES_KEY = "sk-ZOiqskWv6s0hFZC2PQiUgh1517iICkg9e2uIF5B30nNpRtRk"
REPLICATE_KEY = "r8_HTHkWOLjFS2YWpJ0gz8MS9dpYMANfM747e52L"

# ===== НАСТРОЙКИ =====
class Config:
    ADMINS = ["6531356011", "7291492979"]
    IMAGE_LIMIT = 2          # Лимит генерации фото в день
    DIALOG_LIMIT = 7         # Лимит сообщений в диалоге в день
    PREMIUM_PRICE = 20       # Цена в Stars
    PREMIUM_DAYS = 20        # Дней безлимита
...
def check_limits(user) -> tuple:
    """
    Проверяет лимиты пользователя
    Возвращает: (can_image, can_dialog, message)
    """
    today = date.today().isoformat()
    
    # Если премиум — всё можно
    if user.get("premium"):
    ...
    # Проверка лимитов
    can_image = user.get("count_image", 0) < Config.IMAGE_LIMIT
    can_dialog = user.get("count_dialog", 0) < Config.DIALOG_LIMIT
    
    return can_image, can_dialog, None
...
async def handle_callback(callback: types.CallbackQuery):
    ...
    if user_id not in users:
        ...
        # Обновляем username
        users[user_id]["username"] = callback.from_user.username or ""
    ...
    if callback.data == "buy_premium":
        await bot.send_invoice(
            ...
            description=f"{Config.PREMIUM_DAYS} дней безлимитного доступа ко всем функциям",
    ...
    if callback.data == "gen_image":
        ...
        can_image, _, _ = check_limits(user)
        if not user.get("premium") and not can_image:
            await callback.message.answer(
                f"⛔ Лимит фото ({Config.IMAGE_LIMIT}/день) исчерпан!\n"
                f"Купи безлимит за {Config.PREMIUM_PRICE} Stars!"
            )
            return
        user["waiting"] = "image"
...
async def successful_payment(message: Message):
    ...
    await message.answer(
        f"✅ Безлимит активирован на {Config.PREMIUM_DAYS} дней!\n"
        f"Действует до: {users[user_id]['premium_until']}\n"
        f"💰 Оплачено: {Config.PREMIUM_PRICE} Stars"
    )

# ===== /pay 11 — ВКЛЮЧИТЬ БЕЗЛИМИТ (ТОЛЬКО ДЛЯ АДМИНА) =====
...
async def pay_command(message: Message):
    ...
    if code == "11":
        ...
        await message.answer(
            f"✅ Безлимит ВКЛЮЧЕН для админа!\n"
            f"Действует до: {user['premium_until']}"
        )
        ...
        await message.answer("❌ Неверный код. Используй:\n/pay 11 — включить\n/pay 22 — отключить")

# ===== /as77 — СПИСОК ПОЛЬЗОВАТЕЛЕЙ =====
...
async def handle_message(message: Message):
        ...
        if result_url:
            ...
            if watermarked:
                await message.answer_photo(
                    photo=types.BufferedInputFile(watermarked.getvalue(), filename="image.png"),
                    caption=f"✅ Твой человек {prompt}"
                )
            else:
    ...
    if user.get("waiting") in ["dialog_ready", "dialog"] and message.photo:
        if not user["premium"] and user.get("count_dialog", 0) >= Config.DIALOG_LIMIT:
            await message.answer(
                f"⛔ Лимит диалога ({Config.DIALOG_LIMIT}/день) исчерпан!\n"
                f"Купи безлимит за {Config.PREMIUM_PRICE} Stars!"
            )
            return
